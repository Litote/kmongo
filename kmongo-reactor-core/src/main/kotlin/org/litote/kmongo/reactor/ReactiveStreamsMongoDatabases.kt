 ...
 * @param <TResult>      the type of the class to use instead of {@code Document}.
 */
inline fun <reified TResult : Any> MongoDatabase.runCommand(command: String): Mono<TResult> {
 ...
 */
inline fun <reified T : Any> MongoDatabase.dropCollection(): Mono<Void> = dropCollection(defaultCollectionName(T::class))
 ...
 * @mongodb.driver.manual reference/command/drop/ Drop Collection
 */
fun MongoDatabase.dropCollection(collectionName: String): Mono<Void> = emptyResult { getCollection(collectionName).drop() }
