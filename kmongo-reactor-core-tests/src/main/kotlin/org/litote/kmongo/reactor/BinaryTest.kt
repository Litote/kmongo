...
class BinaryTest : KMongoReactorBaseTest<BinaryTest.BinaryFriend>() {
    ...
    fun testUpdate() {
        ...
        col.insertOne(expected).blockLast()
    ...
    fun canMarshallBinary() {
        ...
        col.insertOne(doc).blockLast()
        val count = col.countDocuments("{'_id' : { $binary : 'YWJjZGU=' , $type : '0'}}").block()
