 ...
 *
 */
class ReactiveStreamsDeleteTest : KMongoReactiveStreamsReactorBaseTest<Friend>() {

    @Test
    fun canDeleteASpecificDocument() {
        col.insertMany(listOf(Friend("John"), Friend("Peter"))).blockLast()
        col.deleteOne("{name:'John'}").block()
    ...
    fun canDeleteByObjectId() {
        col.insertOne("{ _id:{${MongoOperator.oid}:'47cc67093475061e3d95369d'}, name:'John'}").block()
