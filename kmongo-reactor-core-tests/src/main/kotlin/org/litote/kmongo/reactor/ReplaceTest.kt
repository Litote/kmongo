 ...
 *
 */
class ReplaceTest : KMongoReactorBaseTest<Friend>() {
    ...
    fun canReplaceTheSameDocument() {
        ...
        col.insertOne(friend).block()
        friend.name = "Johnny"

        col.replaceOne(friend).block()

        val replacedFriend = col.findOne("{name:'Johnny'}").block() ?: throw AssertionError("Value must not null!")
