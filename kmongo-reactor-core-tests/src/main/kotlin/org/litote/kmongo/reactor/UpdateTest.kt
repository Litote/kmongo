 ...
 *
 */
class UpdateTest : KMongoReactorBaseTest<Friend>() {

    @Test
    fun canUpdateMulti() {
        col.insertMany(listOf(Friend("John"), Friend("John"))).blockLast()

        col.updateMany("{name:'John'}", "{$unset:{name:1}}").block()
    ...
    fun canUpdateByObjectId() {
        ...
        col.updateOneById(friend._id!!, "{$set:{name:'John'}}").block()
    ...
    @Test
    fun canUpsert() {
        col.updateOne("{}", "{$set:{name:'John'}}", UpdateOptions().upsert(true)).block()
