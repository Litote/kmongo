 ...
 *
 */
class ReactiveStreamsFindOneAndModifyTest : KMongoReactiveStreamsReactorBaseTest<Friend>() {

    @Test
    fun canFindAndUpdateOne() {
        col.insertOne(Friend("John", "22 Wall Street Avenue")).blockLast()
        col.findOneAndUpdate("{name:'John'}", "{${MongoOperator.set}: {address: 'A better place'}}").block()
    ...
    fun canFindAndUpdateWithNullValue() {
        col.insertOne(Friend("John", "22 Wall Street Avenue")).blockLast()
        col.findOneAndUpdate("{name:'John'}", "{${MongoOperator.set}: {address: null}}").block()
                ?: throw AssertionError("Value cannot null!")
    ...
    fun canFindAndUpdateWithDocument() {
        ...
        col.insertOne(Friend("John", "22 Wall Street Avenue")).blockLast()
        col2.findOneAndUpdate("{name:'John'}", "{${MongoOperator.set}: {address: 'A better place'}}").block()
                ?: throw AssertionError("Value cannot null!")
    ...
    fun canUpsertByObjectId() {
        ...
        ).block() ?: throw AssertionError("Value cannot null!")
