 ...
 *
 */
class FindOneAndModifyTest : KMongoReactorBaseTest<Friend>() {

    @Test
    fun canFindAndUpdateOne() {
        col.insertOne(Friend("John", "22 Wall Street Avenue")).blockLast()
        col.findOneAndUpdate("{name:'John'}", "{$set: {address: 'A better place'}}").block() ?: throw AssertionError("Value cannot null!")
        val savedFriend = col.findOne("{name:'John'}").block() ?: throw AssertionError("Value cannot null!")
    ...
    fun canFindAndUpdateWithNullValue() {
        col.insertOne(Friend("John", "22 Wall Street Avenue")).blockLast()
        col.findOneAndUpdate("{name:'John'}", "{$set: {address: null}}").block() ?: throw AssertionError("Value cannot null!")
        val friend = col.findOne("{name:'John'}").block() ?: throw AssertionError("Value cannot null!")
    ...
    fun canFindAndUpdateWithDocument() {
        ...
        col.insertOne(Friend("John", "22 Wall Street Avenue")).blockLast()
        col2.findOneAndUpdate("{name:'John'}", "{$set: {address: 'A better place'}}").block() ?: throw AssertionError("Value cannot null!")
        val friend = col2.findOne("{name:'John'}").block() ?: throw AssertionError("Value cannot null!")
        assertEquals("John", friend["name"])
    ...
    fun canUpsertByObjectId() {
        ...
        val friend = col.findOneAndUpdate(
                ...
                FindOneAndUpdateOptions().upsert(true).returnDocument(AFTER)).block() ?: throw AssertionError("Value cannot null!")
    ...
    fun canUpsertByStringId() {
        ...
        val friend = col.withDocumentClass<ExposableFriend>().findOneAndUpdate(
                "{_id:${expected._id.json}}",
                "{$setOnInsert: {name: 'John'}}",
                FindOneAndUpdateOptions().upsert(true).returnDocument(AFTER)).block()
        assertEquals(expected, friend)
    }
}
