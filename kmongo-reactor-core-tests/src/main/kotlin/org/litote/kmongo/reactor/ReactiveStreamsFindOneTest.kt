 ...
 *
 */
class ReactiveStreamsFindOneTest : KMongoReactiveStreamsReactorBaseTest<Friend>() {
    ...
    fun canFindOneBson() {
        col.insertOne(Friend("John", "22 Wall Street Avenue")).blockLast()
        val friend =
            col.findOne(Filters.eq("name", "John")).block() ?: throw AssertionError("Value must not null!")
        assertEquals("John", friend.name)
    }
    ...
    fun canFindOneWithObjectIdAsString() {
        ...
        val john = Friend(id, "John")
        col.insertOne(john).blockLast()
        val friend = col.findOne("{_id:{${MongoOperator.oid}:'$id'}}").block()
