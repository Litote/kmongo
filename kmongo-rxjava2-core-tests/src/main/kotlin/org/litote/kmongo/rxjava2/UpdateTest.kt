 ...
 *
 */
class UpdateTest : KMongoRxBaseTest<Friend>() {
    ...
    fun canUpdateByObjectId() {
        ...
        col.updateOneById(friend._id!!, "{$set:{name:'John'}}").blockingGet()
    ...
    @Test
    fun canUpsert() {
        col.updateOne("{}", "{$set:{name:'John'}}", UpdateOptions().upsert(true)).blockingGet()
