...
import org.litote.kmongo.rxjava2.ReactiveStreamsAggregateTest.Article
...
class ReactiveStreamsAggregateTest : KMongoReactiveStreamsRxBaseTest<Article>() {
    ...
    @Test
    fun canAggregate() {
        val list = col.aggregate<Article>("{$match:{}}").toObservable().blockingIterable().toList()
    ...
    fun canAggregateWithManyMatch() {
        val data = col.aggregate<Article>("{$match:{$and:[{tags:'virus'}, {tags:'pandemic'}]}}").toObservable()
    ...
    fun canAggregateWithManyOperators() {
        val data =
            col.aggregate<Article>("[{$match:{tags:'virus'}},{$limit:1}]").toObservable().blockingIterable().toList()
        assertEquals(1, data.size)
    }
