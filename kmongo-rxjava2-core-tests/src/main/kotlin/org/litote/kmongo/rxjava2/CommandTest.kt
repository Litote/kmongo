 ...
 *
 */
class CommandTest : KMongoRxBaseTest<Friend>(oldestMongoTestVersion) {
    ...
    @Test
    fun canRunACommand() {
        val document = database.runCommand<Document>("{ ping: 1 }").blockingGet() ?: throw AssertionError("Document must not null!")
        assertEquals(1.0, document["ok"])
    }

    @Test
    fun canRunACommandWithParameter() {
        col.insertOne("{test:1}").blockingAwait()

        val friends = "friend"

        val document = database.runCommand<Document>("{ count: '$friends' }").blockingGet() ?: throw AssertionError("Document must not null!")
