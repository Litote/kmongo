 ...
 *
 */
class ReactiveStreamsFindOneAndModifyTest : KMongoReactiveStreamsRxBaseTest<Friend>() {

    @Test
    fun canFindAndUpdateOne() {
        col.insertOne(Friend("John", "22 Wall Street Avenue")).blockingAwait()
        col.findOneAndUpdate("{name:'John'}", "{${MongoOperator.set}: {address: 'A better place'}}").blockingGet()
    ...
    fun canFindAndUpdateWithNullValue() {
        col.insertOne(Friend("John", "22 Wall Street Avenue")).blockingAwait()
        col.findOneAndUpdate("{name:'John'}", "{${MongoOperator.set}: {address: null}}").blockingGet()
                ?: throw AssertionError("Value cannot null!")
    ...
    fun canFindAndUpdateWithDocument() {
        ...
        col.insertOne(Friend("John", "22 Wall Street Avenue")).blockingAwait()
        col2.findOneAndUpdate("{name:'John'}", "{${MongoOperator.set}: {address: 'A better place'}}").blockingGet()
                ?: throw AssertionError("Value cannot null!")
    ...
    fun canUpsertByObjectId() {
        ...
        ).blockingGet() ?: throw AssertionError("Value cannot null!")
