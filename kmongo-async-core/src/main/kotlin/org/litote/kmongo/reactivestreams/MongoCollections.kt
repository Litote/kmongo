...
/**
 * Replace a document in the collection according to the specified arguments.
 * Filter the id of the document provided, in order to avoid updated id error.
 *
 * @param filter      the query filter to apply to the replace operation
 * @param replacement the replacement document
 * @param options     the options to apply to the replace operation
 */
inline fun <reified T : Any> MongoCollection<T>.replaceOneByFilteringId(
    filter: Bson,
    replacement: T,
...
fun <T : Any> MongoCollection<T>.replaceOne(
    filter: String,
    replacement: T
): Publisher<UpdateResult> = replaceOne(filter, replacement, ReplaceOptions())
