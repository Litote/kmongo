 ...
 *
 */
class ReactiveStreamSaveTest : KMongoReactiveStreamsCoroutineBaseTest<Friend>() {
    ...
    @Test
    fun `can save one`() = runBlocking {
        col.save(newFriend())
        assertEquals(1, col.countDocuments())
    }

    @Test
    fun `can save one in ClientSession`() = runBlocking {
        mongoClient.startSession().use {
            col.save(it, newFriend())
            assertEquals(1, col.countDocuments(it))
        }
    }

    @Test
    fun `can save one in ClientSession and persists null`() = runBlocking {
        mongoClient.startSession().use {
            col.save(it, newFriend())
            assertEquals(1, col.countDocuments(it))
